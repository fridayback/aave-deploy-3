...
task('withdraw', 'withdraw token from pool')
  ...
  .addParam("amount", "how many tokens to be withdrawed")
  ...
  .setAction(async ({ token, amount,account }) => {
    ...
    // console.log(`withdraw reserve: [${reserve}] for ${amount} for account: ${accounts[account].address}`);
    ...
    console.log(`${accounts[account].address} withdraw ${amount} ${symbol} from pool:`);
    ...
    try {
      const receipt = await (await pool.withdraw(reserve,amountOx,accounts[account].address)).wait(1);
      console.log(`withdraw ${amount} ${symbol} for ${accounts[account].address} success.`);
      console.log(`receipt: \n\ttxHash: ${receipt.transactionHash}\n\tstatus: ${receipt.status}\n\tfrom: ${receipt.from}\n\tto: ${receipt.to}\n\tgasUsed: ${receipt.gasUsed}`);
    } catch (error) {
      console.log(`withdraw ${amount} ${symbol} for ${accounts[account].address} failed.`);
